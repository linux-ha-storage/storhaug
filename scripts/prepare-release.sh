...
LOG=$(git log --format="* %cd %aN <%aE> - ${VERSION}-${RELEASE}%n%b%n%n" --date=local -1 ${RANGE} | sed -r 's/[0-9]+:[0-9]+:[0-9]+ //')
...
rm -rf repo/*/*
mock -r epel-6-x86_64 --resultdir repo/el6/ SRPMS/storhaug-${VERSION}-${RELEASE}.el6.centos.src.rpm
mock -r epel-7-x86_64 --resultdir repo/el7/ SRPMS/storhaug-${VERSION}-${RELEASE}.el7.centos.src.rpm
createrepo repo/el6/
createrepo repo/el7/
